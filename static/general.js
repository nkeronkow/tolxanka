...
function setNSFW() {
    var nsfw = readCookie("show_nsfw");
    if (nsfw === "true") {
        qs(document, "input#nsfw_checkbox").setAttribute("checked", "true");
    }
}

function toggleNSFW(e) {
    var nsfw = (e.target.hasAttribute("checked")) ? "false" : "true";
    createCookie("show_nsfw", nsfw);
...
function main() {
    ...
    addCommentHandlers();
    setNSFW();
    qsael(document, "input#nsfw_checkbox", "click", toggleNSFW, false);
